...
# 应用标题
st.markdown('<h1 class="main-header">🏥 基于机器学习的地中海贫血诊断预测模型</h1>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if not use_default_model:
        ...
        # 尝试加载默认模型
        try:
            model = joblib.load('modelsdzy.pkl')
            st.success("默认模型加载成功!")
        except:
            try:
                with open('modelsdzy.pkl', 'rb') as f:
                    model = pickle.load(f)
                st.success("默认模型加载成功!")
            except Exception as e:
...
with tab1:
    ...
    with col1:
        ...
        S3 = st.number_input('HGB', min_value=-100, max_value=100, value=0, step=1,
                             help="HGB，范围：-100-100")
        S4 = st.number_input('MCV', min_value=-100.0, max_value=100.0, value=0.0, step=0.1,
                             help="MCV，范围：-100.0-100.0")
        S5 = st.number_input('MCH', min_value=-100.0, max_value=100.0, value=0.0, step=0.1,
                             help="MCH，范围：-100.0-100.0")

        st.markdown('</div>', unsafe_allow_html=True)

    with col2:
        st.markdown('<div class="input-section">', unsafe_allow_html=True)
        S6 = st.number_input('MCHC', min_value=-100.0, max_value=1000.0, value=0.0, step=0.1,
                             help="MCHC，范围：-100.0-1000.0")
        S7 = st.number_input('RDWCV', min_value=-100.0, max_value=1000.0, value=0.0, step=0.1,
                             help="RDWCV，范围：-100.0-1000.0")
        S8 = st.number_input('RDWSD', min_value=-100.0, max_value=1000.0, value=0.0, step=0.1,
                             help="RDWSD，范围：-100.0-1000.0")
        S9 = st.number_input('FERR', min_value=-100.00, max_value=100000.00, value=0.00, step=0.01,
                             help="FERR，范围：-100.00-100000.00")
        st.markdown('</div>', unsafe_allow_html=True)
